...
```text
Notion (KO 9개 DB + EN 9개 DB) ──fetch──▶  content/*.json + content/**/*.mdx  ──vite build──▶  dist/public  ──▶  Cloudflare Pages
...
- i18n: KO 원본 DB + 병렬 EN DB(`Slug`/`Key` 매칭) → `content/i18n/en.json`, KO/EN 토글
...
3. 영어 노출이 필요하면 짝이 되는 `… (EN)` DB 항목을 채웁니다. 장문 콘텐츠는 `Slug`, 짧은 설정 섹션은 `Key`로 매칭됩니다.
...
variable NOTION_*_DB_ID   # KO/EN DB id (기본값은 코드에 내장)
```
