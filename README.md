...
```text
Notion (`Portfolio Entries` 단일 DB) ──fetch──▶  content/*.json + content/**/*.mdx  ──vite build──▶  dist/public  ──▶  Cloudflare Pages
...
- i18n: 같은 DB에서 `Locale=ko/en`, 같은 `Key`로 매칭 → `content/i18n/en.json`, KO/EN 토글
...
1. Notion `KYS — Portfolio (CMS)`의 `Portfolio Entries` 데이터베이스에서 항목을 추가/수정합니다.
2. `Section`, `Locale`, `Key`, `Status`, `Order`를 채웁니다. 사이트에는 `Status=Published`만 반영됩니다.
